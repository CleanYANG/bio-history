...
type WritingKey = "papyrus" | "waxTablet" | "paper";
...
const options: Record<WritingKey, WritingOption> = {
  papyrus: {
    label: "Papyrus",
    resultTitle: "Widely used for writing.",
    resultText: "A familiar surface for keeping words.",
  },
  waxTablet: {
    label: "Wax tablet",
    resultTitle: "Reusable and practical.",
    resultText: "Useful for notes that may change.",
  },
  paper: {
    label: "Paper",
    resultTitle: "Not yet in use in this world.",
    resultText: "It belongs elsewhere, and later.",
  },
};
...
export default function RomanWritingPage() {
  const [step, setStep] = useState(0);
  const [selected, setSelected] = useState<WritingKey | null>(null);

  const currentOption = selected ? options[selected] : null;

  const handleSelect = (key: WritingKey) => {
    setSelected(key);
    setStep(2);
  };
  ...
  return (
    <main className="min-h-screen bg-[var(--background)] px-6 py-8 text-[var(--foreground)] sm:px-8 lg:px-12">
      <div className="mx-auto flex min-h-[calc(100vh-4rem)] w-full max-w-2xl flex-col justify-center">
        ...
        {step === 0 && (
          <Card title="You are writing something-Rome Empire">
            <p>You need to record something.</p>
            <p>It must be kept for later.</p>
            <div className="mt-8">
        ...
        {step === 1 && (
          <Card title="What do you use?">
            <div className="mt-2 grid gap-3">
              {(Object.entries(options) as Array<
                [WritingKey, WritingOption]
              >).map(([key, option]) => (
                <button
                  key={key}
        ...
        {step === 2 && currentOption && (
          <Card title={currentOption.resultTitle}>
            ...
            <p className="mt-4">
              Not everything familiar today existed in the past.
            </p>
